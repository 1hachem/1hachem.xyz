import Layout from "@/components/Layouts/BlogLayout";

export const meta = {
  title: "About me",
  topics: [
    "about",
    "cericulem vitae",
    "experiece",
    "skills",
    "projects",
    "life style",
  ],
};

<img src="/images/cover.png" className="h-28 md:h-64" />
## Hi there, I'm Hachem Betrouni

## Currently

<p className="text-sm">sep 2022 - today </p>I am working as CTO and AI engineer
at [BIGmama Technology](https://www.big-mama.io/).{" "}

We are building [Hyko.ai](https://www.hyko.ai/) a tool that would make it easier to understand
the needs of our clients and build automatic AI solutions for them !

I am also doing my final year at the [National Polytechnic School of Algiers](https://www.enp.edu.dz/)
soon I will graduate as an AI engineer.

## Future direction

I am learning fullstack development to bootstrap digital tools (I want to start solo-startups ✨).

I am also looking forward to joining [DeepMind]("https://www.deepmind.com/") as an AI engineer in the near future.

## Previously

<p className="text-sm">May 2023</p>
Helped orgnaizing IndabaX here in Algeria.

<p className="text-sm">aug 2022</p>I attended Indaba and AIhack in Tunisia.

<p className="text-sm">feb 2022 - aug 2022</p>I worked for 6 months as an AI
research intern at [InstaDeep](https://www.instadeep.com/">, trying to predict
and generate new variants of Sars-CoV-2 using GANs (which failed miserably 🥹).
huge thank you to all the InstaDeepers for the amazing experience, I learned
many things with you !

<p className="text-sm">Nov 2021</p>
Won 3rd place at Google's devfest2021 Hackathon.

<p className="text-sm">Oct 2021</p>I started freelancing on [Upwork]() in
machine learning and data science.

<p className="text-sm">Jul 2021- sep 2021</p>Won a scholarship from Samsung to
join Samsung innovation campus in AI and machine learning.

<p className="text-sm">Nov 2020</p> Came top 2% national wide, pursued a degree in
Data science AI at the National Polytechnic School of Algiers.

<p className="text-sm">Sep 2018-Nov 2020</p> Attended engineering preparatory classes,
learned the basics of mathematics, physics, thermodynamics, chemistry, computer science,
fluid mechanics ... here I discovered my passion to problem solving and to multidisciplinary
engineering !

## Fun facts

- Happily married at the age of 22.
- [I never owned a smartphone](/blogs/smartphone).
- I did my elementary schooling at UAE.
- I have 1 heart, 1 nose and 1 kidney.

export default ({ children }) => <Layout meta={meta}>{children}</Layout>;
